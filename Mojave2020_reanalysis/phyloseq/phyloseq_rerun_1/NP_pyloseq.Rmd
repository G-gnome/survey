...
###STEP1: Load all necessary packages for analysis More information
about Phyloseq can be found at the following link:
[Phyloseq](https://joey711.github.io/phyloseq/) If you get error in this
step, you probably need to install any packages which causes error.
...
####Function to summarize taxonomy plot_taxa_summary(GlobalPatterns,
"Phylum") plot_taxa_summary(GlobalPatterns, "Phylum", "SampleType")
summarize_taxa(GlobalPatterns, "Phylum") summarize_taxa(GlobalPatterns,
"Phylum", "SampleType")
...
###STEP2: Import Mapping file (metadate file) 1.Check mapping file
before import to R, R doesn't seem to like sample name to start with
number or contain "-" in sample name. If you get error in this step, you
should check file name first.

2.First column of first row should not start with #, R will not read the
first row that starts with \#

3.  You can choose which samples to include in analysis by indicating
    "KEEP" in Description column
...
###STEP3: Check if your metadata file has been import successfully and
correctly
...
OTU table from Mojave 16S data is "MC2017FC.otu_table.txt". MC-Mojave
Crust-2017-FC-Fungi completed
...
###STEP6: Import taxonomy table Taxonmy table generated from AMPtk need
to be rearranged using following script.
...
###STEP7: Import phylogenetic tree Phylogenetic tree can also be include
...
###STEP8: Construct Phyloseq object To construct phyloseq object, otu
...
###STEP10: Plot read counts to check dataset Check read counts: any
samples that have very low reads should be removed.
...
###STEP11: Rarefaction curve Extract OTU table, transpose and convert to
data frame
...
Extract OTU table from phyloseq object as data frame and use vegan to
...
###STEP11.1: Plot Alpha diversity by Crust_type Alpha diversity can be
...
```{r}
sample_data(physeq.prune.rarefy)$Site_layer = factor(sample_data(physeq.prune.rarefy)$Site_layer, levels = c("AB_SF","ODLO_SF", "CIMA_SF", "AB_Sub","ODLO_Sub", "CIMA_Sub"))
```
...
Checking homoscedasticity. If Levene's test p value \< 0.05, data are
heteroscedastic and need to use Welch correction.

instead of "physeq.prune.rarefy.anova.Crusttype.SF = aov(Observed \~
Crust_type, data.anova)" we will do oneway.test "oneway.test(Observed \~
Crust_type, data = data.anova, var.equal = F)"
...
```{r}
ylabel <- data.frame("ylabel" = c(245,280,235,345,215,295))
```
...
```{r warning=FALSE}
physeq.prune.rarefy.ps.plot.richness.site.layer = plot_richness(physeq.prune.rarefy, x="Site_layer", color=("Site_layer"), measures=c("Chao1")) + geom_boxplot() + ggtitle("Mojave Biocrust Fungal Alpha Diversity (Observed) by Site and Layer") + stat_compare_means(method = "anova", label.y = 400) + stat_compare_means(aes(label=..p.signif..), method = "t.test", ref.group = "GMT_Sub", label.y = 380, hide.ns = TRUE) + theme_bw() + theme(plot.title = element_text(hjust = 0.5))

physeq.prune.rarefy.ps.plot.richness.site.layer
```

To save Alpha diversity (Chao1) by Site plot as a pdf into your current
working directory.
...
```{r warning=FALSE}
pdf("./Figures/Fig2A_Fungal_Alpha_Div_by_Site_reorder_stie.pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.Site
dev.off()
```
...
physeq.prune.rarefy.SF = subset_samples(physeq.prune.rarefy, Layer=="Surface")
sample_data(physeq.prune.rarefy.SF)$Site = factor(sample_data(physeq.prune.rarefy.SF)$Site, levels = c("CIMA", "ODLO", "AB"))
sample_data(physeq.prune.rarefy.SF)$Crust_type = factor(sample_data(physeq.prune.rarefy.SF)$Crust_type, levels = c("LAC", "CLC", "GLC", "RMC","SMC", "Dirt", "SMC.ster"))
```
...
```{r}
APS = subset_taxa(physeq.prune.rarefy.SF, Class == "Glomeromycetes")

physeq.prune.rarefy.plot.richness.Site.class = plot_richness(APS, x="Crust_type", color=("Crust_type"), measures=c("Chao1")) + geom_boxplot(lwd=0.5) + ggtitle("Glomeromycetes") + stat_compare_means(method = "anova") + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_colour_brewer(palette="Set1")

physeq.prune.rarefy.plot.richness.Site.class
```

```{r}
APS = subset_taxa(physeq.prune.rarefy.SF, Genus == "Naganishia")

physeq.prune.rarefy.plot.richness.Site.class = plot_richness(APS, x="Crust_type", color=("Crust_type"), measures=c("Chao1")) + geom_boxplot(lwd=0.5) + ggtitle("Naganishia") + stat_compare_means(method = "anova") + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_colour_brewer(palette="Set1")

physeq.prune.rarefy.plot.richness.Site.class
```

```{r}
APS = subset_taxa(physeq.prune.rarefy.SF, Genus == "Coniochaeta")

physeq.prune.rarefy.plot.richness.Site.class = plot_richness(APS, x="Crust_type", color=("Crust_type"), measures=c("Chao1")) + geom_boxplot(lwd=0.5) + ggtitle("Coniochaeta") + stat_compare_means(method = "anova") + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_colour_brewer(palette="Set1")

physeq.prune.rarefy.plot.richness.Site.class
```

```{r}
APS = subset_taxa(physeq.prune.rarefy.SF, Genus == "Rhodotorula")

physeq.prune.rarefy.plot.richness.Site.class = plot_richness(APS, x="Crust_type", color=("Crust_type"), measures=c("Observed")) + geom_boxplot(lwd=0.5) + ggtitle("Rhodotorula") + stat_compare_means(method = "anova") + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_colour_brewer(palette="Set1")

physeq.prune.rarefy.plot.richness.Site.class
```

```{r}
APS = subset_taxa(physeq.prune.rarefy.SF, Class == "Mortierellomycetes")

physeq.prune.rarefy.plot.richness.Site.class = plot_richness(APS, x="Crust_type", color=("Crust_type"), measures=c("Observed")) + geom_boxplot(lwd=0.5) + ggtitle("Mortierellomycetes") + stat_compare_means(method = "anova") + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_colour_brewer(palette="Set1")

physeq.prune.rarefy.plot.richness.Site.class
```

```{r}
APS = subset_taxa(physeq.prune.rarefy.SF, Genus == "Exophiala")

physeq.prune.rarefy.plot.richness.Site.class = plot_richness(APS, x="Crust_type", color=("Crust_type"), measures=c("Observed")) + geom_boxplot(lwd=0.5) + ggtitle("Exophiala") + stat_compare_means(method = "anova") + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + scale_colour_brewer(palette="Set1")

physeq.prune.rarefy.plot.richness.Site.class
```

```{r warning=FALSE}
pdf("./Figures/FigS1B_Sordariomycetes.pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.Site.class
dev.off()
```
...
```{r}
physeq.prune.rarefy.plot.richness.Site.SUB = plot_richness(physeq.prune.rarefy.SUB, x="Site", color=("Site"), measures=c("Chao1")) + geom_boxplot(lwd=1) + ggtitle("Mojave Biocrust Fungal Alpha Diversity by Site") + stat_compare_means(method = "anova", label.y = 280) + theme(plot.title = element_text(hjust = 0.5)) + geom_text(data=LABELS, aes(x=LABELS$Site, y = 270 , label = LABELS$Letters)) + theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
```{r warning=FALSE}
pdf("./Figures/Fig2C_Fungal_Alpha_Div_by_Layer_surface_left.pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.Layer
dev.off()
```
...
```{r}
ylabel <- data.frame("ylabel" = c(215,95,225))
```
...
```{r warning=FALSE}
pdf("./Figures/Fig2E_Fungal_Alpha_Div_by_Crust_type_reorder_type.pdf", width = 6, height = 3.8 )
physeq.prune.rarefy.plot.richness.Crust_type
dev.off()
```
...
###Alpha div by crust type ###Reorder Crust_type

```{r}
#physeq.prune.rarefy.SF = subset_samples(physeq.prune, Layer=="Surface")
#sample_data(physeq.prune.rarefy.SF)$Crust_type = factor(sample_data(physeq.prune.rarefy.SF)$Crust_type, levels = c("LAC","CLC", "GLC", "RMC", "SMC", "Dirt", "SMC.ster"))
sample_data(physeq.prune.rarefy)$Crust_type = factor(sample_data(physeq.prune.rarefy)$Crust_type, levels = c("SMC", "Dirt", "SMC.Sub", "SMC.ster"))
```
...
```{r}
ylabel <- data.frame("ylabel" = c(219,320,90,215))
```
...
for (i in FungiClass) {
  ...
  APS.plot = plot_richness(APS, x="Site", color =("Site"), measures=c("Observed")) + geom_boxplot(lwd=1,fill=c("#999999", "gold2", "cadetblue3")) + ggtitle(i) + stat_compare_means(method = "anova") + theme(plot.title = element_text(hjust = 0.5)) + scale_colour_manual(values = c("#000000", "#E69F00", "#56B4E9"))
  print(APS.plot)
}
dev.off()
```
...
```{r}
pdf("./Figures/Fungal_Class_Alpha_(Observed)_Diversity_by_Layer_surface_left.pdf", width = 6, height = 3.8)
...
```{r}
APS = subset_taxa(physeq.prune.rarefy, Class == "Mortierellomycetes")
APS.plot = plot_richness(APS, x="Layer", color =("Layer"), measures=c("Observed")) + geom_boxplot(lwd=0.5) + ggtitle("Mortierellomycetes") + stat_compare_means(method = "t.test", label.x.npc = c("right")) + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + scale_colour_brewer(palette="Set1") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
print(APS.plot)
```

```{r}
APS = subset_taxa(physeq.prune.rarefy, Genus == "Naganishia")
APS.plot = plot_richness(APS, x="Layer", color =("Layer"), measures=c("Observed")) + geom_boxplot(lwd=0.5) + ggtitle("Naganishia") + stat_compare_means(method = "t.test", label.x.npc = c("right")) + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + scale_colour_brewer(palette="Set1") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
print(APS.plot)
```

```{r}
# Filter data for the desired taxon (e.g., "Naganishia")
APS = subset_taxa(physeq.prune.rarefy, Genus == "Naganishia")

# Plot alpha diversity by biocrust type
APS.plot = plot_richness(APS, x = "Crust_type", color = "Crust_type", measures = c("Observed")) +
  geom_boxplot(lwd = 0.5) +
  ggtitle("Rhodotorula") +
  stat_compare_means(method = "t.test", label.x.npc = c("right")) +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_bw() +
  scale_colour_brewer(palette = "Set1") +
...
```{r}
# Filter data for the desired taxon (e.g., "Rhodotorula")
APS = subset_taxa(physeq.prune.rarefy, Genus == "Rhodotorula")
...
```{r}
APS = subset_taxa(physeq.prune.rarefy, Genus == "Rhodotorula")
APS.plot = plot_richness(APS, x="Layer", color =("Layer"), measures=c("Observed")) + geom_boxplot(lwd=0.5) + ggtitle("Rhodotorula") + stat_compare_means(method = "t.test", label.x.npc = c("right")) + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + scale_colour_brewer(palette="Set1") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
print(APS.plot)
```

```{r}
APS = subset_taxa(physeq.prune.rarefy, Genus == "Coniochaeta")
APS.plot = plot_richness(APS, x="Layer", color =("Layer"), measures=c("Observed")) + geom_boxplot(lwd=0.5) + ggtitle("Coniochaeta") + stat_compare_means(method = "t.test", label.x.npc = c("right")) + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + scale_colour_brewer(palette="Set1") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
print(APS.plot)
```

```{r}
# Filter data for the desired taxon (e.g., "Coniochaeta")
APS = subset_taxa(physeq.prune.rarefy, Genus == "Coniochaeta")
...
```{r}
APS = subset_taxa(physeq.prune.rarefy, Genus == "Exophiala")
APS.plot = plot_richness(APS, x="Layer", color =("Layer"), measures=c("Observed")) + geom_boxplot(lwd=0.5) + ggtitle("Exophiala") + stat_compare_means(method = "t.test", label.x.npc = c("right")) + theme(plot.title = element_text(hjust = 0.5)) + theme_bw() + scale_colour_brewer(palette="Set1") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
print(APS.plot)
```

```{r}
# Filter data for the desired taxon (e.g., "Exophiala")
APS = subset_taxa(physeq.prune.rarefy, Genus == "Exophiala")
...
```{r}
APS = subset_taxa(physeq.prune.rarefy.SF, Class == "Tremellomycetes")
alpha.diversity.Sordariomycetes = estimate_richness(APS, measures = c("Observed"))
data.anova.Sordariomycetes = cbind(sample_data(APS), alpha.diversity.Sordariomycetes)
physeq.prune.rarefy.anova = aov(Observed ~ Crust_type, data.anova.Sordariomycetes)
summary(physeq.prune.rarefy.anova)
```
...
######To save taxa summary into text file under your current working
directory.
...
```{r fig.height=5, fig.width=8, fig.align="center"}
taxcom_layer = ggplot(data = psmelt(physeq.prune.rarefy), mapping = aes_string(x = "Crust_type" ,y = "Abundance", fill = "Class" )) + 
  geom_bar(stat="identity", position="fill") + 
  ggtitle("Fungal Taxonomic Composition (Class level) by Crust_type")+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())
print(taxcom_layer)
```
...
```{r}
png("./Figures/Fig3A_Fungal_Taxonomic_Composition_(Class_level)_by_Layer_surface_left.png", units="in", width = 8, height = 5.8, res = 300 )
taxcom_layer
dev.off()
```
...
```{r}
physeq.prune.rarefy.SF = subset_samples(physeq.prune.rarefy, Layer=="Surface")
physeq.prune.rarefy.SF.top20 = prune_taxa(names(sort(taxa_sums(physeq.prune.rarefy.SF), TRUE))[1:20], physeq.prune.rarefy.SF)
physeq.prune.rarefy.SF.top20
```

`{r} tax_table(physeq.prune.rarefy.SF.top20)`sd
